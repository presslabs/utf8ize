...
function utf8ize_generator() {
	...
	foreach ( $tables as $table ) {
		...
		foreach ( $indexes as $index ) {
			if ( 'FULLTEXT' != $index['Index_type'] ) continue;
			...
			if ( ! preg_match( '/utf8_/', $column['Collation'] ) ) {
				foreach ( $fulltext as $index_name => $index ) {
					if ( in_array( $column['Field'], $index ) ) {
						...
						$_fulltext[] = "ALTER TABLE `$table` ADD FULLTEXT `$index_name` ("
							. join( ', ', array_map( create_function( '$s', 'return "`$s`";' ), $index ) ) . ")";
		...
		foreach ( $columns as $column ) {
			...
			if ( ! preg_match( '/utf8_/', $column['Collation'] ) ) {
				...
				if ( preg_match( '/^(ENUM|SET)/', $type ) ) {
					$null = ( 'NO' == $column['Null'] ? 'NOT NULL' : 'NULL' );
